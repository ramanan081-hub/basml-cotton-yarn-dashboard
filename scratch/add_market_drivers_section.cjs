...
// 1. Add calculation hooks and data
const targetHooks = `  const [selectedDistricts, setSelectedDistricts] = useState([]);
  const millIntelligence = data.millIntelligence || initialData.yarns.millIntelligence || [];`;

const replacementHooks = `  const [selectedDistricts, setSelectedDistricts] = useState([]);
...
if (content.includes(targetHooks)) {
  ...
  console.log('Successfully added hooks and spreadData to App.jsx.');
} else {
  console.error('Target hooks block not found.');
}

// 2. Add Section 1.5 between Section 1 and Section 2
const targetSectionSplit = `       </div>

       {/* SECTION 2: PRODUCTION CHARTS */}`;

const replacementSectionSplit = `       </div>

      {/* SECTION 1.5: YARN PROFITABILITY SPREAD & PRICE MOVEMENT DRIVERS */}
...
const normalizedSectionSplit = targetSectionSplit.replace(/\r\n/g, '\n');
const normalizedReplacementSectionSplit = replacementSectionSplit.replace(/\r\n/g, '\n');

if (content.includes(normalizedSectionSplit)) {
  content = content.replace(normalizedSectionSplit, normalizedReplacementSectionSplit);
  console.log('Successfully inserted Section 1.5 in App.jsx.');
} else {
  console.error('Section split target not found.');
}
