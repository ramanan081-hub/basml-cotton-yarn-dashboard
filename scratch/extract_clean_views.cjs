...
async function scan() {
  ...
  for await (const line of rl) {
    try {
      ...
      // and it was before step 1920, parse it!
      if (step.type === 'VIEW_FILE' && step.content) {
        if (lastTargetIsApp && lastCallStepIndex < 1920) {
  ...
  fs.writeFileSync('./scratch/recovered_clean_350_970.txt', output.join('\n'));
  console.log('Saved recovered clean lines to ./scratch/recovered_clean_350_970.txt');
}
