...
export default function CottonDashboard({ data, darkMode, colors }) {
  ...
  return (
    <div className="space-y-gutter">
      ...
      {/* Indian Cotton Consumption & Sectoral Usage Share */}
      <div className="card-chart-green rounded-xl p-6">
        <h3 className="text-lg font-headline font-black text-primary uppercase tracking-tight mb-4 border-b border-outline-variant pb-2 flex items-center gap-2">
          <span className="material-symbols-outlined text-xl">pie_chart</span>
          National Cotton Consumption & Sectoral Usage Share
        </h3>
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-center">
          <div className="lg:col-span-5 h-[280px] font-mono relative">
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                <Pie
                  data={data.indianCotton?.consumptionShares || []}
                  cx="50%"
                  cy="50%"
                  innerRadius={50}
                  outerRadius={85}
                ...
                >
                  {(data.indianCotton?.consumptionShares || []).map((entry, index) => (
                ...
                <Tooltip
                  ...
                  formatter={(value, name, props) => [
                    `${value}% (${props.payload.bales} Lakh Bales)`,
                    `Est. Value: ₹${props.payload.marketValueCr.toLocaleString('en-IN')} Cr`
                  ]}
                />
              </PieChart>
            </ResponsiveContainer>
          ...
          <div className="lg:col-span-7 space-y-4">
            <div className="text-xs text-on-surface-variant leading-relaxed">
              India's total domestic cotton demand averages <strong className="text-on-surface font-bold">314 Lakh Bales</strong> annually. 
              The chart highlights how raw cotton fiber and yarn supply flows are distributed across industrial, commercial, and government sectors.
            </div>
            
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {(data.indianCotton?.consumptionShares || []).map((item, idx) => (
                <div key={idx} className="bg-surface-container-low/60 border border-outline-variant/15 rounded-xl p-3.5 flex flex-col justify-between hover:bg-surface-container-low/90 transition-colors">
                  <div className="flex justify-between items-center mb-1">
                    <span className="font-bold font-mono text-xs text-on-surface">{item.category}</span>
                    <span className="text-xs font-black font-mono text-primary">{item.value}%</span>
                  </div>
                  <div className="flex justify-between items-center text-[11px] font-mono text-on-surface-variant">
                    <span>Volume: {item.bales} Lakh Bales</span>
                    <span className="font-bold text-emerald-500">₹{item.marketValueCr.toLocaleString('en-IN')} Cr</span>
                  </div>
                </div>
              ))}
            </div>
            
            <div className="p-3 bg-surface-container-low/60 rounded-xl border border-outline-variant/10 text-[10px] font-mono leading-relaxed text-on-surface-variant">
              <strong>Usage Highlight:</strong> Mills Usage (Private Spinners) consumes <strong>72%</strong> of all cotton fiber to spin yarn. Export & Industrial usage is <strong>15%</strong> (including cotton/yarn exports to Bangladesh/China, non-apparel and medical uses). Government Reserves (8%) represents minimum support price buffers, and Public & Direct Retail (5%) represents direct cottage handloom weavers.
            </div>
          </div>
        </div>
      </div>
